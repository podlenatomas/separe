...
export default function EventsTable() {
  ...
  return (
    <section
      ...
      className="bg-[#1A1A1A] text-[#F5F5F0] border-b border-white/10 scroll-mt-24"
    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-32">
        ...
        <motion.p
          className="text-[#F5F5F0]/60 font-light leading-[1.8] max-w-xl mb-12 text-pretty"
          {...fade}
        >
          {t("description")}
        </motion.p>
        ...
        <div className="bg-white/10 rounded-sm overflow-hidden flex overflow-x-auto snap-x snap-mandatory hide-scrollbar [mask-image:linear-gradient(to_right,black_85%,transparent_100%)] md:[mask-image:none] md:grid md:grid-cols-3 gap-px">
          {EVENT_KEYS.map((key, i) => (
            ...
            >
              ...
              <h3 className="text-xs md:text-sm font-black tracking-tight text-[#F5F5F0] mb-1 text-balance">
                {t(`blocks.${key}.title`)}
              </h3>
              <p className="text-[9px] md:text-[10px] font-light uppercase tracking-[0.1em] text-[#F5F5F0]/40 mb-4">
                {t(`blocks.${key}.schedule`)}
              </p>
              <p className="text-[11px] md:text-xs font-light text-[#F5F5F0]/60 leading-relaxed text-pretty">
                {t(`blocks.${key}.desc`)}
              </p>
            </motion.div>
          ))}
        </div>
        <div className="md:hidden text-center text-[10px] uppercase tracking-widest text-[#F5F5F0]/30 my-4 flex items-center justify-center gap-2">
          <span className="animate-pulse">←</span> {t("swipeHint")}{" "}
